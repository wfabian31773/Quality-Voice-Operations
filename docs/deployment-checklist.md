...
| `STRIPE_PRICE_STARTER_AI_MINUTES` | Stripe metered Price ID for the per-minute AI overage on the Starter tier. Required in production as of Task #1321 — the per-tier metered-pricing migration is complete in every environment, and a missing value would make `GET /billing/upgrade-preview` silently quote the catalog overage rate (`shared/billing/planCatalog.ts`) instead of the live Stripe rate. `validateBillingConfig` also flips this to a hard error at admin-api boot when `STRIPE_METER_EVENT_AI_MINUTES` is set. | Stripe Dashboard > Products > Price ID (metered) | `price_...` |
...
When `STRIPE_METER_EVENT_AI_MINUTES` is set (i.e. this deployment has opted into per-tier metered AI-minute billing — which, as of Task #1321, every production deployment is required to do), the verifier *also* checks the three `STRIPE_PRICE_<TIER>_AI_MINUTES` metered prices and asserts each one has `recurring.usage_type=metered` AND `recurring.meter` equal to `STRIPE_METER_AI_MINUTES`. **Note the two distinct env vars:** `STRIPE_METER_EVENT_AI_MINUTES` is the meter *event name* the platform sends to `meterEvents.create({ event_name })`, while `STRIPE_METER_AI_MINUTES` is the Stripe Billing meter *id* (e.g. `mtr_…`) that Stripe stores on each metered Price's `recurring.meter` field — they are not interchangeable, and an opted-in deployment must set both. If `STRIPE_METER_EVENT_AI_MINUTES` is set but `STRIPE_METER_AI_MINUTES` is not, every metered row fails fast with a `missing-env` status pointing at the meter-id var. This catches the failure mode where a typoed price id, a wrong-meter wiring, or a licensed price wired here by mistake silently keeps the upgrade-preview overage quote on the catalog rate (a `mixed` provenance the BillingEstimator's "Live Stripe rate" badge would *not* flag). Deployments that have not yet opted in (i.e. `STRIPE_METER_EVENT_AI_MINUTES` unset) skip these rows entirely; once a deployment opts in, missing `STRIPE_PRICE_<TIER>_AI_MINUTES` values are now hard validation failures: `scripts/validate-env.ts` lists them as production-required so the validator exits non-zero, and `validateBillingConfig()` returns them in `errors` so admin-api startup also exits before binding the port (Task #1321 promoted this from a soft warning).

> **Automated as a deploy gate.** `.replit`'s `[deployment].build` chains `npm run verify:stripe-prices` after the type-checks but **before** the slow Vite build, so any non-zero exit (missing env var, wrong-interval wiring, wrong-usage-type wiring, wrong-meter wiring, deleted Stripe price, network failure to Stripe) **fails the publish before the new image goes live** — and fails fast, without spending a minute building a frontend that would have shipped against a broken billing config. You no longer need to remember to run it by hand for production deploys; just make sure `STRIPE_SECRET_KEY`, the six `STRIPE_PRICE_*` base vars, and (once opted in) **both** `STRIPE_METER_EVENT_AI_MINUTES` and `STRIPE_METER_AI_MINUTES` plus the three `STRIPE_PRICE_<TIER>_AI_MINUTES` metered vars are set in the deployment's secrets before clicking Publish.
>
> **Re-run on demand from the admin console.** Platform admins can re-run the same check from **Platform Admin → Billing config health** (powered by `GET /platform/billing-config-health`) without redeploying. Useful when ops rotates a Stripe price ID, suspects a typo, or is investigating a sudden live-rate badge regression. The admin tile renders the metered AI-minute rows in their own table beneath the base prices, with the per-minute rate (sub-cent precision when present), the catalog comparison, and the meter binding so a misconfigured row is spot-checkable at a glance.
...
STRIPE_METER_EVENT_AI_MINUTES="ai_minutes" \
STRIPE_METER_AI_MINUTES="mtr_..." \
STRIPE_PRICE_STARTER_AI_MINUTES="price_..." \
STRIPE_PRICE_PRO_AI_MINUTES="price_..." \
STRIPE_PRICE_ENTERPRISE_AI_MINUTES="price_..." \
npm run verify:stripe-prices
```

Each base row prints the resolved Stripe price, its `unit_amount`, the monthly-equivalent (annual ÷ 12), and the catalog comparison. Each metered AI-minute row prints the per-minute rate (with sub-cent precision via `unit_amount_decimal`), the meter binding, and the catalog overage comparison. The script exits non-zero if any env var is missing, any base price points at the wrong interval (e.g. `_ANNUAL` wired to a monthly price), any metered price is not actually metered or is wired to the wrong meter, or Stripe fails to retrieve the price.
