/**
 * Browser-level smoke test for high-traffic Platform Admin pages.
 *
 * The Sales Inbox filters spec (`tests/e2e/salesInboxFilters.spec.ts`) is a
 * deep regression that only exercises one page. This spec is the wide
 * counterpart: it boots no fixtures of its own, and just walks through the
 * other admin pages most likely to break silently between PRs (Platform
 * Admin dashboard, Marketplace, Compliance, Global Analytics, Governance,
 * Sales Inbox initial render, Billing).
 *
 ...
 *   4. No uncaught page errors fired during navigation.
 *
 ...
 * if a 5xx took down the whole page, assertions 2 or 3 will catch it.
 * The warnings still land in the spec log for triagers.
 *
 ...
 *
 *   npx tsx tests/e2e/adminPagesSmoke.spec.ts
 *
 * Pre-requisites (same as the Sales Inbox spec):
...
const SPEC_NAME = 'admin-pages-smoke';

interface PageCheck {
  /** Path under BASE_URL — e.g. `/admin/dashboard`. */
  path: string;
...
const PAGES: PageCheck[] = [
  // Platform Admin dashboard (Tenants tab is the default landing). Heading
  // comes from the i18n key `admin:platform_admin.page_title` which renders
  // as "Platform Administration".
  { path: '/admin/dashboard', slug: 'platform-admin-dashboard', expect: { kind: 'heading', text: 'Platform Administration' } },

  // Sales Inbox initial render — covers the lazy chunk + first list fetch.
  // The deep filter behavior is in the dedicated salesInboxFilters spec.
  { path: '/admin/sales-inbox', slug: 'sales-inbox', expect: { kind: 'heading', text: 'Sales Inbox' } },

  // Marketplace Management (PageHeader title="Marketplace Management").
  { path: '/admin/marketplace', slug: 'admin-marketplace', expect: { kind: 'heading', text: 'Marketplace Management' } },

  // Compliance & Security (PlatformCompliance — h1 "Compliance & Security").
  { path: '/admin/security', slug: 'platform-compliance', expect: { kind: 'heading', text: 'Compliance & Security' } },

  // Global Analytics (PageHeader title="Global Analytics").
  { path: '/admin/analytics', slug: 'admin-analytics', expect: { kind: 'heading', text: 'Global Analytics' } },

  // Governance default tab routes to Evolution Engine (h1 from its own
  // PageHeader: "Platform Evolution Engine").
  { path: '/admin/governance', slug: 'admin-governance', expect: { kind: 'heading', text: 'Platform Evolution Engine' } },

  // Billing — admin-org tenant has an enterprise subscription seeded by
  // seed-admin, so the Billing h1 must render.
  { path: '/admin/billing', slug: 'admin-billing', expect: { kind: 'heading', text: 'Billing' } },
];
...
async function checkPage(page: Page, check: PageCheck): Promise<PageFailure | null> {
  ...
  try {
    ...
    // Auth check: the PlatformAdminGuard kicks non-admins back to /dashboard
    // and the global redirect bounces logged-out users to /login.
    ...
    if (/\/login(\?|$)/.test(landed)) {
      ...
      // PlatformAdminGuard redirect to /dashboard would land us off-route.
...
async function run(): Promise<void> {
  ...
  if (failures.length > 0) {
    console.error(`[e2e] ${failures.length} of ${PAGES.length} admin page(s) failed:`);
    for (const f of failures) {
      console.error(`[e2e]   - ${f.page.path}: ${f.reason}`);
    }
    assert(false, `${failures.length} admin page smoke check(s) failed`);
  }

  console.log(`[e2e] PASS — all ${PAGES.length} admin pages rendered cleanly`);
}
