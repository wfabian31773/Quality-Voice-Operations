 ...
 * discount headline produced by `buildPortalDiscountHeadline`. Skips
 * when `STRIPE_SECRET_KEY` / `DATABASE_URL` are unset and refuses to
 * run against a non-test Stripe key.
 *
 * Run: npm run test:e2e:billing-portal-discount-headline
 */
...
const SPEC_NAME = 'billing-portal-discount-headline';
...
async function restoreSubscription(
  ...
  if (!snapshot) {
    // Nothing was there before; remove the row we created so we don't
    // leave a stale Stripe customer id pointing at a now-deleted
    // test-mode customer.
    await pool
...
async function seed(pool: pg.Pool, stripe: Stripe): Promise<SeedResult> {
  ...
  // Run id makes the headline tail unique per run.
  const couponName = `E2E Portal ${runId}`;
  ...
  const customer = await stripe.customers.create({
    email: `e2e-portal-${runId.toLowerCase()}@voiceaihub.dev`,
    description: `E2E ${SPEC_NAME} runId=${runId}`,
    metadata: { e2e: SPEC_NAME, runId, tenantId: ADMIN_TENANT_ID },
  });

  // The `coupon` parameter on the customer endpoint was removed in
  // API version `2025-04-30.basil`. Pin this one write to an older
  // version via the `Stripe-Version` header so the legacy field is
  // still honored; reads continue to use the project's pinned version.
  // `fetch` is used because the SDK does not let `rawRequest` override
  // `Stripe-Version` per call.
  ...
  // between the apply (older API) and the read (pinned API) fails
  // here, not as a slow Playwright timeout.
  ...
  if (!probed) {
    throw new Error(
      `loadActiveCustomerDiscount returned null for customer ${customer.id}; production code path will not surface a headline`,
    );
  }
...
async function cleanup(
  ...
  // repeated CI runs don't accumulate test-mode configurations on the
  // Stripe account. PortalConfigCleanupScheduler also sweeps these,
  // but doing it inline keeps the dashboard clean even if the
  // scheduler isn't running in the e2e environment.
  try {
    const cfgs = await stripe.billingPortal.configurations.list({ limit: 100 });
    for (const cfg of cfgs.data) {
      if (cfg.metadata?.couponId === seedData.couponId && cfg.active) {
...
async function run(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[e2e] seeded customer=${seedData.customerId} coupon=${seedData.couponId} runId=${seedData.runId}`,
    );
    ...
    console.log('[e2e] PASS — discount headline rendered on Stripe portal');
  } catch (err) {
