/**
 * Task #1394 — Browser-level regression for the post-checkout
 * `billing-annual-switch-success-banner` chip block when the
 * subscription response only carries the legacy `discount` field
 * (no `discounts[]` array). The sibling spec
 * `billingAnnualSwitchSuccessStackedDiscounts.spec.ts` covers the
 * stacked branch; this one exercises the legacy fallback in the
 * IIFE's `discountList` so a regression that drops the
 * `sub?.discount ? [sub.discount] : []` arm doesn't slip past CI by
 * rendering zero chips on older server builds.
 *
 ...
 *
 * Run:  npm run test:e2e:billing-annual-switch-success-legacy-discount
 *
...
const SPEC_NAME = 'billing-annual-switch-success-legacy-discount';
...
// Singular tooltip + sessionStorage key mirrored from Billing.tsx —
// keep in sync if either moves. The plural variant is exercised by
// the sibling stacked-discounts spec.
...
async function seed(pool: pg.Pool): Promise<SeedResult> {
  ...
  const fixtureEmail = `billing-annual-switch-success-legacy-e2e-${runId}@voiceaihub.dev`;
...
// Fully-active annual subscription with ONLY the legacy `discount`
// field populated (no `discounts[]`) so the success banner exercises
// the IIFE's `sub?.discount ? [sub.discount] : []` fallback. A
// regression that drops that fallback would render zero chips here
// even though the legacy server is still returning a discount.
function buildAnnualLegacyDiscountSubscriptionResponse(): unknown {
  const now = new Date();
  ...
  try {
    ...
    await context.route('**/api/billing/subscription', async (route: Route) => {
      await route.fulfill({
        ...
        body: JSON.stringify(buildAnnualLegacyDiscountSubscriptionResponse()),
      });
    });
    ...
    assert(
      chipCount === 1,
      `expected exactly one annual-switch success-banner discount chip from the legacy fallback, got ${chipCount}`,
    );
