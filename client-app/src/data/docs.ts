...
5. Always confirm the callback phone number before ending.`, 'text'),
  ...
  },
  {
    ...
    description: 'Tune voice, pacing, fillers, interruption behavior, and pick the right voice for each language.',
    readTime: '5 min',
    updated: '2026-04-27',
    body: [
      h2('Voice catalog'),
      ...
      h2('Voice and language compatibility'),
      p('All eleven OpenAI voices can speak any language QVO supports, but some sound noticeably more natural than others depending on the language. The Voice & Agent Config panel filters the dropdown into two groups based on the language you selected:'),
      ul([
        '★ Recommended — voices tuned for the active language. They appear under a "Recommended for <Language>" group and are starred so they\'re easy to spot.',
        'Other voices — still selectable, but grouped under "Other voices (may sound less natural)" so admins know quality may suffer.',
      ]),
      p('If you pick a voice that is not in the recommended group, the panel highlights the dropdown in amber and shows an inline warning suggesting better alternatives — no save is blocked, but you have an explicit heads-up before publishing.'),
      tip('When you change the language, the existing voice is kept if it is still in the recommended set. Otherwise the warning appears so you can pick a better-matched voice before publishing.'),
      h3('Recommended voices by language'),
      ul([
        'English (en) — alloy, ash, ballad, coral, echo, fable, onyx, nova, sage, shimmer, verse (all voices work well).',
        'Spanish (es) — coral, nova, shimmer, sage, alloy, verse.',
        'French (fr) — nova, shimmer, sage, alloy, coral, verse.',
        'German (de) — alloy, nova, sage, shimmer, verse.',
        'Portuguese (pt) — nova, shimmer, alloy, coral, sage.',
        'Italian (it) — nova, shimmer, sage, alloy, coral, verse.',
        'Dutch (nl) — alloy, nova, shimmer, sage.',
        'Chinese (zh) — alloy, nova, shimmer.',
        'Japanese (ja) — alloy, nova, shimmer.',
        'Korean (ko) — alloy, nova, shimmer.',
        'Arabic (ar) — alloy, nova, shimmer.',
        'Hindi (hi) — alloy, nova, shimmer.',
      ]),
      info('Asian and Arabic-language voices are limited to alloy, nova, and shimmer because the other voices have noticeable English accents in those languages. We refresh the recommendations as OpenAI ships voice updates.'),
      h2('Pacing'),
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/salesforce-app-manager.svg', alt: 'Salesforce Setup App Manager with a New Connected App button highlighted in the top-right corner', caption: 'Setup\u2009→\u2009App Manager\u2009→\u2009New Connected App.' },
      { type: 'image', src: '/docs/screenshots/salesforce-oauth-settings.svg', alt: 'Salesforce New Connected App OAuth settings showing Enable OAuth Settings ticked, the QVO Callback URL filled in, and the api and refresh_token/offline_access scopes selected', caption: 'Enable OAuth, paste the Callback URL, and add the api + refresh_token/offline_access scopes.' },
      { type: 'image', src: '/docs/screenshots/salesforce-consumer-details.svg', alt: 'Salesforce Consumer Details page showing Consumer Key and Consumer Secret values with Copy buttons', caption: 'Manage Consumer Details — copy the Consumer Key (Client ID) and Consumer Secret.' },
      ...
      { type: 'image', src: '/docs/screenshots/salesforce-sandbox-toggle.svg', alt: 'QVO Salesforce connector card with a Login host selector showing Production and Sandbox options, and a Sign in with Salesforce button', caption: 'Pick Production or Sandbox to match your org type before signing in.' },
      ...
      { type: 'image', src: '/docs/screenshots/salesforce-task-on-contact.svg', alt: 'Salesforce Contact record with a QVO Voice Agent Call activity logged on the Activity timeline including a transcript snippet and link', caption: 'A Task with the AI summary appears on the matching Contact or Lead activity timeline.' },
      h3('Common issues'),
      issues([
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/outlook-app-registration.svg', alt: 'Microsoft Entra Register an application page with the Name field set to QVO Voice Agent, Single tenant selected, and a Web Redirect URI containing the QVO callback', caption: 'Pick Single tenant (or Multi-tenant), then paste the QVO callback as the Web Redirect URI.' },
      { type: 'image', src: '/docs/screenshots/outlook-overview-ids.svg', alt: 'Microsoft Entra app Overview page with Application (client) ID and Directory (tenant) ID highlighted with copy buttons', caption: 'Copy Application (client) ID and Directory (tenant) ID from the Overview page.' },
      ...
      { type: 'image', src: '/docs/screenshots/outlook-client-secret.svg', alt: 'Microsoft Entra Certificates & secrets tab with a newly generated client secret and the Value column highlighted next to a Copy button', caption: 'Copy the Value column NOW — it is masked after you leave the page.' },
      ...
      { type: 'image', src: '/docs/screenshots/outlook-api-permissions.svg', alt: 'Microsoft Entra API permissions panel showing the five required Microsoft Graph delegated permissions ticked plus a Grant admin consent button', caption: 'Add the five Graph permissions, then click Grant admin consent.' },
      ul([
      ...
      { type: 'image', src: '/docs/screenshots/outlook-qvo-calendar-pick.svg', alt: 'QVO Outlook Calendar integration card showing Connected status with a dropdown to pick which calendar QVO may book on', caption: 'After OAuth returns, pick the calendar QVO should write events to.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/hubspot-create-app.svg', alt: 'HubSpot Developers Manage apps page with a Create app button highlighted in the top-right', caption: 'Developer hub → Manage apps → Create app.' },
      { type: 'image', src: '/docs/screenshots/hubspot-oauth-redirect.svg', alt: 'HubSpot app Auth tab showing Client ID, Client secret, and the Redirect URLs field highlighted with the QVO callback URL', caption: 'Paste the QVO callback URL into the Auth tab\'s Redirect URLs field.' },
      { type: 'image', src: '/docs/screenshots/hubspot-scopes.svg', alt: 'HubSpot scopes panel with the five required scopes ticked: contacts read/write, deals read/write, and timeline', caption: 'Tick exactly the five scopes QVO needs.' },
      ...
      { type: 'image', src: '/docs/screenshots/hubspot-qvo-connect.svg', alt: 'QVO HubSpot connector card with two options: Sign in with HubSpot (OAuth) on the left and a paste-an-access-token form on the right', caption: 'QVO\u2009→\u2009Integrations\u2009→\u2009HubSpot offers OAuth (recommended) or an access token.' },
      ...
      { type: 'image', src: '/docs/screenshots/hubspot-private-app.svg', alt: 'HubSpot private app Auth tab showing the access token starting with pat- highlighted next to a Copy button', caption: 'Private app access tokens begin with pat- and never expire — rotate manually.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/google-enable-api.svg', alt: 'Google Cloud Console API library page for Google Calendar API with the ENABLE button highlighted', caption: 'Library\u2009→\u2009Google Calendar API\u2009→\u2009Enable.' },
      ...
      { type: 'image', src: '/docs/screenshots/google-oauth-consent.svg', alt: 'Google OAuth consent screen with Internal user-type selected and the two calendar scopes ticked', caption: 'Pick Internal where possible, then add the two calendar scopes.' },
      ...
      { type: 'image', src: '/docs/screenshots/google-credentials.svg', alt: 'Google Cloud Create OAuth client ID form with Application type set to Web application and the QVO redirect URI in Authorized redirect URIs', caption: 'Use Web application + the QVO callback URL — no trailing slash.' },
      { type: 'image', src: '/docs/screenshots/google-client-secret.svg', alt: 'Google Cloud OAuth client created modal showing the Client ID and Client secret values with copy buttons', caption: 'Copy both Client ID and Client secret immediately — the secret is only shown once.' },
      ...
      { type: 'image', src: '/docs/screenshots/google-qvo-calendar-picker.svg', alt: 'QVO Google Calendar connector card showing the connected account, calendar picker, timezone, and buffer-time settings', caption: 'Pick the calendar, timezone, and buffer times in the QVO connector card.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/slack-create-app.svg', alt: 'Slack Create an app dialog with the From scratch option selected and an app-name field reading QVO', caption: 'Pick "From scratch" and name the app QVO.' },
      ...
      { type: 'image', src: '/docs/screenshots/slack-bot-scopes.svg', alt: 'Slack OAuth & Permissions page with Bot Token Scopes chat:write, channels:read, and groups:read ticked', caption: 'Add the three Bot Token Scopes — chat:write, channels:read, groups:read.' },
      ...
      { type: 'image', src: '/docs/screenshots/slack-redirect-urls.svg', alt: 'Slack Redirect URLs section with the QVO callback URL added and a Save URLs button highlighted', caption: 'Click Save URLs — the URL is not active until saved.' },
      ...
      { type: 'image', src: '/docs/screenshots/slack-app-credentials.svg', alt: 'Slack Basic Information App Credentials panel with Client ID and Client Secret fields highlighted', caption: 'Copy Client ID and Client Secret from the Basic Information page.' },
      ...
      { type: 'image', src: '/docs/screenshots/slack-channel-id.svg', alt: 'Slack channel About panel showing the Channel ID C091ABCD234 with a Copy button next to it', caption: 'Channel ID lives at the bottom of the channel About panel — click Copy.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/pipedrive-create-app.svg', alt: 'Pipedrive Developer Hub Create a new app dialog with Public app selected and an app name field reading QVO Voice Agent', caption: 'Pick Public app for multi-tenant or Private app for one company.' },
      { type: 'image', src: '/docs/screenshots/pipedrive-callback-url.svg', alt: 'Pipedrive OAuth & access page with the Callback URL field highlighted containing the QVO callback', caption: 'Paste the QVO callback URL into the Callback URL field.' },
      h3('2. Set scopes'),
      ul([
      ...
      { type: 'image', src: '/docs/screenshots/pipedrive-scopes.svg', alt: 'Pipedrive Scopes & permissions panel with contacts:full, deals:full, activities:full, and users:read ticked', caption: 'Tick exactly these four scopes — QVO needs nothing else.' },
      ...
      { type: 'image', src: '/docs/screenshots/pipedrive-qvo-connect.svg', alt: 'QVO Pipedrive connector card with two options: Sign in with Pipedrive on the left and a personal API token form on the right', caption: 'OAuth on the left, personal API token on the right.' },
      ...
      { type: 'image', src: '/docs/screenshots/pipedrive-personal-token.svg', alt: 'Pipedrive Settings Personal preferences API page with the personal API token highlighted next to a Copy button and a company domain field', caption: 'Copy the personal API token plus your company domain (yourcompany).' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'image', src: '/docs/screenshots/quickbooks-create-app.svg', alt: 'Intuit Developer Create an app dialog with QuickBooks Online + Payments selected and an app name field reading QVO Voice Agent', caption: 'My Apps\u2009→\u2009Create an app\u2009→\u2009QuickBooks Online + Payments.' },
      ...
      { type: 'image', src: '/docs/screenshots/quickbooks-keys-environment.svg', alt: 'Intuit Developer Keys & OAuth tab with the Development environment selected, Client ID and Client Secret shown, and the QVO redirect URI added', caption: 'Pick the environment first — keys are different for Development vs Production.' },
      { type: 'image', src: '/docs/screenshots/quickbooks-scopes.svg', alt: 'Intuit Developer scopes page with com.intuit.quickbooks.accounting ticked and other scopes left unticked', caption: 'com.intuit.quickbooks.accounting is the only scope QVO needs.' },
      ...
      { type: 'image', src: '/docs/screenshots/quickbooks-realm-pick.svg', alt: 'Intuit Authorize QVO modal with a Choose a company list showing two QuickBooks realms and the production company selected', caption: 'Pick the realm (company) — QVO captures the Realm ID from the callback.' },
      ...
      { type: 'image', src: '/docs/screenshots/quickbooks-qvo-invoice-config.svg', alt: 'QVO QuickBooks connector card showing the connected realm, Invoice Item ID, item description, default invoice amount, and a Sync now button', caption: 'Set Invoice Item ID and default amount, then click Sync now.' },
