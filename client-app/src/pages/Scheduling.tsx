        ...
        {hours.map(hour => {
          ...
          return (
            <div key={hour} className="flex border-b border-border min-h-[60px]">
              <div className="w-16 shrink-0 text-xs text-text-muted p-2 text-right">
              ...
              <div className="flex-1 p-1 space-y-1">
                {hourBookings.map(b => (
                  <button key={b.id} onClick={() => setShowDetailModal(b)}
                    ...
                    {b.provider_name && <span className="text-text-muted ml-1">- {b.provider_name}</span>}
                  </button>
                ))}
              </div>
            </div>
  ...
  const renderWeekView = () => {
    ...
    return (
      <div className="grid grid-cols-7">
        {dayNames.map(d => (
          <div key={d} className="px-2 py-2 text-center text-xs font-medium text-text-muted border-b border-border">{d}</div>
        ))}
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className="min-h-[120px] border-b border-r border-border p-1.5">
              <div className={`text-xs font-medium mb-1 ${isToday ? 'text-primary font-bold' : 'text-text-muted'}`}>
                {day.getDate()}
              </div>
              <div className="space-y-0.5">
                ...
                {dayBookings.length > 4 && (
                  <div className="text-[10px] text-text-muted px-1.5">+{dayBookings.length - 4} more</div>
                )}
              </div>
            </div>
          );
        ...
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className={`min-h-[80px] border-b border-r border-border p-1 ${!isCurrentMonth ? 'opacity-40' : ''}`}>
              <div className={`text-xs font-medium mb-0.5 ${isToday ? 'text-primary font-bold' : 'text-text-muted'}`}>
                {day.getDate()}
              </div>
              <div className="space-y-0.5">
                ...
                {dayBookings.length > 3 && (
                  <div className="text-[10px] text-text-muted px-1">+{dayBookings.length - 3} more</div>
                )}
              </div>
            </div>
          );
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[800px]">
          <div className="grid" style={{ gridTemplateColumns: `150px repeat(${days.length}, 1fr)` }}>
            <div className="border-b border-r border-border p-2 text-xs font-medium text-text-muted">Provider</div>
            {days.map((day, i) => (
              <div key={i} className="border-b border-r border-border p-2 text-center text-xs font-medium text-text-muted">
            ...
            {viewProviders.map(provider => (
              <React.Fragment key={`row-${provider.id}`}>
                <div className="border-b border-r border-border p-2">
                  ...
                  {provider.specialty && <div className="text-[10px] text-text-muted">{provider.specialty}</div>}
                </div>
                {days.map((day, di) => {
                  ...
                  return (
                    <div key={`${provider.id}-${di}`} className="border-b border-r border-border p-1 min-h-[60px]">
                      ...
                      {dayBookings.length > 3 && <div className="text-[9px] text-text-muted">+{dayBookings.length - 3}</div>}
                    </div>
                  );
                })}
              </React.Fragment>
          ...
          <div key={p.id} className="bg-surface border border-border rounded-xl p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                {p.specialty && <p className="text-xs text-text-muted mt-0.5">{p.specialty}</p>}
                {p.location && <p className="text-xs text-text-muted flex items-center gap-1 mt-1"><MapPin className="h-3 w-3" />{p.location}</p>}
                {p.email && <p className="text-xs text-text-muted mt-0.5">{p.email}</p>}
              </div>
              {!isReadOnly && (
                <div className="flex gap-1">
                  <button onClick={() => { setEditingProvider(p); setProviderFormData({ name: p.name, specialty: p.specialty, email: p.email, phone: p.phone, location: p.location }); setShowProviderForm(true); }}
                    className="p-1 rounded hover:bg-surface-secondary text-text-muted"><Edit2 className="h-3.5 w-3.5" /></button>
          ...
          <div key={r.id} className="bg-surface border border-border rounded-xl p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-text-muted mt-0.5 capitalize">{r.type}{r.location ? ` - ${r.location}` : ''}</p>
              </div>
              {!isReadOnly && (
                <div className="flex gap-1">
        ...
        {waitlist.map(w => (
          <div key={w.id} className="flex items-center justify-between p-3 rounded-lg bg-surface border border-border">
            <div>
              ...
              {w.notes && <div className="text-xs text-text-muted mt-0.5">{w.notes}</div>}
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-surface border border-border rounded-xl p-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm"><span className="text-text-muted">Fill Rate</span><span className="font-medium text-heading">{s.fill_rate ?? 0}%</span></div>
              <div className="flex justify-between text-sm"><span className="text-text-muted">No-Show Rate</span><span className="font-medium text-heading">{s.no_show_rate ?? 0}%</span></div>
              <div className="flex justify-between text-sm"><span className="text-text-muted">Cancellation Rate</span><span className="font-medium text-heading">{s.cancellation_rate ?? 0}%</span></div>
              <div className="flex justify-between text-sm"><span className="text-text-muted">Avg Hours to Appt</span><span className="font-medium text-heading">{s.avg_hours_to_appointment ?? 0}h</span></div>
            </div>
          </div>
            ...
            <div className="space-y-1.5 max-h-40 overflow-auto">
              {reportData.by_provider.map((p, i) => (
                <div key={i} className="flex justify-between text-xs">
                  <span className="text-text-muted truncate">{p.provider_name || 'Unassigned'}</span>
              ...
              {reportData.by_type.map((t, i) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span className="text-text-muted truncate">{t.type_name || 'Untyped'}</span>
                  </span>
          ...
          <div className="bg-surface border border-border rounded-xl p-4">
            ...
            <div className="space-y-1.5">
              {reportData.cancellation_reasons.map((r, i) => (
                <div key={i} className="flex justify-between text-xs">
                  <span className="text-text-muted">{r.cancellation_reason}</span>
            ...
            <div className="flex flex-wrap gap-3">
              {reportData.by_source.map((s, i) => (
                <div key={i} className="bg-surface-secondary rounded-lg px-3 py-2 text-center">
                  ...
                  <div className="text-[10px] text-text-muted capitalize">{s.booking_source || 'unknown'}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="space-y-6">
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          <div className="space-y-3">
              ...
              return (
                <div key={p.id}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-text-muted">{p.booked_hours || 0}h | {p.total_bookings} appts | {p.completed} done | {p.no_shows} no-show</span>
                  </div>
            ...
            {utilizationData.provider_utilization.length === 0 && <p className="text-sm text-text-muted text-center py-4">No provider data</p>}
          </div>
        </div>
              ...
              <div key={r.id} className="bg-surface-secondary rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[10px] text-text-muted capitalize">{r.type}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-heading">{r.booked_hours || 0}h</div>
                    <div className="text-[10px] text-text-muted">{r.total_bookings} bookings</div>
                  </div>
                </div>
              </div>
            ))}
            {utilizationData.resource_utilization.length === 0 && <p className="text-sm text-text-muted col-span-full text-center py-4">No resource data</p>}
          </div>
        </div>
      </div>
    );
  ...
  const renderRulesTab = () => (
    <div className="space-y-6">
      <div className="bg-surface border border-border rounded-xl p-4">
        ...
        <p className="text-xs text-text-muted mb-4">Configure rules that govern how bookings are created. Rules can be set globally or per appointment type.</p>
        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-colors ${
              activeTab === tab.id ? 'bg-primary text-on-primary' : 'bg-surface-secondary text-text-muted hover:text-heading'
            }`}>
            {tab.icon}{tab.label}
          </button>
        ))}
            ...
            <div className="flex items-center justify-between p-3 border-b border-border flex-wrap gap-2">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative">
                  <Search className="h-3.5 w-3.5 absolute left-2 top-1/2 -translate-y-1/2 text-text-muted" />
                ...
                <div className="flex gap-0.5 bg-surface-secondary rounded-lg p-0.5">
                  {(['day', 'week', 'month', 'agenda', 'resource'] as ViewMode[]).map(v => (
                    <button key={v} onClick={() => setView(v)}
                      className={`px-2.5 py-1 rounded-md text-[10px] font-medium transition-colors capitalize ${view === v ? 'bg-primary text-on-primary' : 'text-text-muted hover:text-heading'}`}>
                      {v}
                    </button>
                  ))}
                </div>
        ...
        <Modal open onClose={() => setShowForm(false)} ariaLabel={editingBooking ? 'Edit Booking' : 'New Booking'} panelClassName="bg-surface border border-border rounded-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-text-muted mb-1">Appointment Type</label>
      ...
      {showDetailModal && (
        <Modal open onClose={() => setShowDetailModal(null)} ariaLabel="Appointment Details" panelClassName="bg-surface border border-border rounded-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-border">
              ...
              <button onClick={() => setShowDetailModal(null)} className="text-text-muted hover:text-heading"><X className="h-5 w-5" /></button>
            </div>
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><span className="text-text-muted">Date:</span> <span className="text-heading font-medium">{formatDate(showDetailModal.start_time)}</span></div>
                <div><span className="text-text-muted">Time:</span> <span className="text-heading font-medium">{formatTime(showDetailModal.start_time)} - {formatTime(showDetailModal.end_time)}</span></div>
                {showDetailModal.provider_name && <div><span className="text-text-muted">Provider:</span> <span className="text-heading font-medium">{showDetailModal.provider_name}</span></div>}
                {showDetailModal.appointment_type_name && <div><span className="text-text-muted">Type:</span> <span className="text-heading font-medium">{showDetailModal.appointment_type_name}</span></div>}
                {showDetailModal.resource_name && <div><span className="text-text-muted">Resource:</span> <span className="text-heading font-medium">{showDetailModal.resource_name}</span></div>}
                {showDetailModal.location && <div><span className="text-text-muted">Location:</span> <span className="text-heading font-medium">{showDetailModal.location}</span></div>}
                {showDetailModal.contact_name && <div><span className="text-text-muted">Contact:</span> <span className="text-heading font-medium">{showDetailModal.contact_name}</span></div>}
                {showDetailModal.contact_phone && <div><span className="text-text-muted">Phone:</span> <span className="text-heading font-medium">{showDetailModal.contact_phone}</span></div>}
                {showDetailModal.contact_email && <div><span className="text-text-muted">Email:</span> <span className="text-heading font-medium">{showDetailModal.contact_email}</span></div>}
                <div><span className="text-text-muted">Source:</span> <span className="text-heading font-medium capitalize">{showDetailModal.booking_source}</span></div>
              </div>

              {showDetailModal.description && (
                <div><span className="text-xs text-text-muted">Description:</span><p className="text-sm text-heading mt-0.5">{showDetailModal.description}</p></div>
              )}
              {showDetailModal.notes && (
                <div><span className="text-xs text-text-muted">Notes:</span><p className="text-sm text-heading mt-0.5">{showDetailModal.notes}</p></div>
              )}
      ...
      {showTypeForm && (
        <FormModal title={editingType ? 'Edit Appointment Type' : 'New Appointment Type'} onClose={() => setShowTypeForm(false)} onSave={saveType}>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-text-muted mb-1">Buffer (min)</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2 pt-5">
              ...
              <label className="text-xs text-text-muted">Allow Self-Scheduling</label>
            </div>
          </div>
        </FormModal>
      )}
    ...
    <div className="space-y-3">
      ...
      {showForm && (
        <div className="bg-surface-secondary rounded-lg p-3 space-y-2">
          <div>
            <label className="block text-[10px] font-medium text-text-muted mb-0.5">Appointment Type (blank = global)</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-medium text-text-muted mb-0.5">Min Lead Time (hours)</label>
            ...
            <div>
              <label className="block text-[10px] font-medium text-text-muted mb-0.5">Max Lead Time (days)</label>
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-1 text-xs rounded bg-surface-secondary text-text-muted">Cancel</button>
    ...
    <div className="space-y-3">
      {reminders.map(r => (
        <div key={r.id} className="flex items-center justify-between bg-surface-secondary rounded-lg p-3 text-xs">
          <div>
            ...
            <span className="text-text-muted ml-2">via {r.channel} | {timingLabel(r.timing_minutes)} before</span>
            {r.appointment_type_name && <span className="text-text-muted ml-2">| {r.appointment_type_name}</span>}
          </div>
          {!isReadOnly && (
      ...
      {showForm && (
        <div className="bg-surface-secondary rounded-lg p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-medium text-text-muted mb-0.5">Type</label>
            ...
            <div>
              <label className="block text-[10px] font-medium text-text-muted mb-0.5">Channel</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-medium text-text-muted mb-0.5">Timing (minutes before)</label>
              <input type="number" value={formData.timing_minutes} onChange={e => setFormData(p => ({ ...p, timing_minutes: parseInt(e.target.value) || 60 }))}
                className="w-full px-2 py-1 text-xs rounded border border-border bg-surface text-heading" />
            </div>
            <div>
              <label className="block text-[10px] font-medium text-text-muted mb-0.5">Appointment Type</label>
              <select value={formData.appointment_type_id} onChange={e => setFormData(p => ({ ...p, appointment_type_id: e.target.value }))}
                className="w-full px-2 py-1 text-xs rounded border border-border bg-surface text-heading">
                <option value="">All types</option>
                {appointmentTypes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
          ...
          <div>
            <label className="block text-[10px] font-medium text-text-muted mb-0.5">Template</label>
