          ...
          <div className="flex flex-col lg:flex-row gap-12">
            <aside className="lg:w-64 shrink-0">
              <div className="lg:sticky lg:top-24">
                <button
                  className="lg:hidden flex items-center gap-2 text-sm font-medium text-text-primary mb-4"
                ...
                <nav className={`${tocOpen ? 'block' : 'hidden'} lg:block`}>
                  <h3 className="font-display text-xs font-semibold text-text-primary/40 uppercase tracking-wider mb-4">
                    In this guide
                  </h3>
                    ...
                    {guide.content.map((section, idx) => (
                      <li key={idx}>
                        <button
                          ...
                          className="text-left w-full text-sm text-text-primary/60 hover:text-primary font-body px-3 py-1.5 rounded-lg hover:bg-primary/5 transition-colors"
                        >
                          {section.title}
                        </button>
                      </li>
                ...
                <RevealSection key={idx}>
                  <div id={`section-${idx}`} className="mb-12 scroll-mt-24">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-8 h-8 rounded-lg bg-primary text-white font-display text-sm font-bold flex items-center justify-center">
                        {idx + 1}
                      </div>
                    ...
                    <div className="prose prose-slate max-w-none">
                      {section.body.split('\n\n').map((paragraph, pIdx) => {
                          ...
                          return (
                            <ul key={pIdx} className="space-y-2 my-4">
                              {items.map((item, iIdx) => {
                                ...
                                if (match) {
                                  return (
                                    <li key={iIdx} className="flex gap-2 text-sm text-text-primary/70 font-body leading-relaxed">
                        ...
                        return (
                          <p key={pIdx} className="text-sm text-text-primary/70 leading-relaxed font-body mb-4">
                            {paragraph}
                          </p>
                        );
                      })}
                    ...
                    {section.code && (
                      <div className="mt-4 bg-sidebar-bg rounded-xl p-5 overflow-x-auto">
                  ...
                  {adjacent.prev ? (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-text-primary/40 font-body">Previous</p>
                        <p className="text-sm font-semibold text-text-primary truncate">{adjacent.prev.title}</p>
                      </div>
                    </Link>
                  ) : (
                    <div />
                  ...
                  {adjacent.next ? (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-sm font-semibold text-text-primary truncate">{adjacent.next.title}</p>
                      </div>
                      <ArrowRight className="h-4 w-4 text-text-primary/40 group-hover:text-primary transition-colors shrink-0" />
                    </Link>
                  ) : (
                    <div />
                  )}
