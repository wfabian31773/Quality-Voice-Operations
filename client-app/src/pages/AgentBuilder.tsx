...
import {
  ...
  AlertTriangle, Keyboard, Search, Pause, Volume2, Loader2,
} from 'lucide-react';
...
function VoicePicker({
  ...
  const others = useMemo(() => VOICES.filter((v) => !recommendedSet.has(v)), [recommendedSet]);
  ...
  const playPreview = useCallback(async (v: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/agents/voice-preview', {
        ...
        credentials: 'include',
        headers,
        body: JSON.stringify({ voice: v, language, greeting: greetingTrimmed }),
      });
  ...
  const renderRow = (v: string, isInRecommendedGroup: boolean) => {
    ...
    const previewLabel = isPlaying ? t('voicePreviewStop') : t('voicePreviewPlay', { voice: v });
    return (
      <li key={v}>
        <div
        ...
        >
          ...
          >
            ...
            {isInRecommendedGroup && (
              <span className="text-amber-500 dark:text-amber-400 text-[10px]" aria-hidden>★</span>
            )}
          </button>
          <button
            type="button"
        ...
        {hasError && (
          <p className="mt-1 text-[10px] text-red-500 dark:text-red-400" role="alert">
            {t('voicePreviewError')}
          </p>
        )}
      </li>
