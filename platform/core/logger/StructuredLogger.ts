...
export class StructuredLogger {
  ...
  /** Merge additional fields into the global context applied to all loggers. */
  ...
  forTenant(tenantId: string): StructuredLogger {
    return new StructuredLogger(this.component, { ...this.baseContext, tenantId });
  }

  private formatLog(level: LogLevel, message: string, context: LogContext = {}): string {
    const merged: LogContext = {
      ...StructuredLogger.globalContext,
      ...this.baseContext,
      ...context,
    };
  ...
  retryAttempt(context: {
    ...
    this.warn(`Retry ${context.attempt}/${context.maxAttempts}: ${context.operation}`, {
      event: 'retry_attempt',
      ...context,
    });
  }
