...
describe('OutlookCalendarConnectorAdapter retry-with-backoff (BL-014 Task #505)', () => {
  ...
  // dispatch budget — were unverified for Outlook Calendar until now.
  test(
    ...
    async () => {
      ...
      vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
        ...
        // created Graph event as if the earlier failures never happened.
        if (attempt === 1) {
          return new Response(
            JSON.stringify({ error: { code: 'BadGateway', message: 'bad gateway' } }),
            { status: 502 },
          );
        }
        if (attempt === 2) {
          return new Response(
            JSON.stringify({ error: { code: 'ServiceUnavailable', message: 'unavailable' } }),
            { status: 503 },
          );
        }
        return new Response(
          JSON.stringify({ id: 'evt-5xx', webLink: 'https://outlook.office.com/calendar/item/5xx' }),
          { status: 200 },
        );
      }));
      ...
      expect(result.meta).toMatchObject({ eventId: 'evt-5xx', provider: 'outlook-calendar' });
  ...
  test(
    ...
    async () => {
      ...
      vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
        const method = (init?.method ?? 'GET').toUpperCase();
        calls.push({ url, method });
        // Persistent 503 — the Graph helper MUST cap at 3 attempts and
        // let the adapter convert the final failed Response into a clean
        // {success:false, error:'Microsoft Graph API error 503: ...'}
      ...
      expect(result.error).toMatch(/Microsoft Graph API error 503/);
