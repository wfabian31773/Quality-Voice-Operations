...
describe('ZohoConnectorAdapter retry-with-backoff (BL-014 Task #505)', () => {
  ...
  // BL-014 (Task #981): the 429 test above only covered the Retry-After
  // path. The remaining helper guarantees — recovery from transient 5xx,
  // clean exhaustion, and the 3-attempt cap that pins us inside the 60s
  // dispatch budget — were unverified for Zoho until now.
  test(
    'recovers from a transient 5xx (502 → 503 → 200) on the Calls POST and ultimately succeeds',
    { timeout: 15_000 },
    async () => {
      const calls: Array<{ url: string; method: string }> = [];
      let attempt = 0;
      vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
        ...
        // BEFORE the adapter even sees them; the third attempt succeeds
        // with a normal Zoho write response.
        if (attempt === 1) {
        ...
        return new Response(
          JSON.stringify({
            data: [{ code: 'SUCCESS', status: 'success', details: { id: 'call-5xx' } }],
          }),
          { status: 200 },
        );
      }));
      ...
      expect(result.meta).toMatchObject({ provider: 'zoho', callId: 'call-5xx' });
      // Two 5xx + one successful retry, all targeting the same /Calls POST.
  ...
  test(
    ...
    async () => {
      ...
      vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
        const method = (init?.method ?? 'GET').toUpperCase();
        calls.push({ url, method });
        // Persistent 503 — Zoho's helper MUST cap at 3 attempts and let
        // the adapter convert the final failed Response into a clean
        // {success:false, error:'Zoho 503: ...'} result rather than
        // looping or hanging until the per-dispatch deadline fires.
      ...
      expect(result.error).toMatch(/Zoho 503/);
      expect(result.externalId).toBeUndefined();
      // Exactly three /Calls POSTs: the helper does NOT make a 4th
